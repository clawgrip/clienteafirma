...
class TestCAdESMimeType {
	...
	void cadesPorDefecto() throws Exception {
		...
		Assertions.assertNull(mimeType, "No se debe agregar el mimetype a las firmas CAdES por defecto"); //$NON-NLS-1$
	}
	 ...
	 */
	@Test
	void cadesPorDefectoIndicandoMimeType() throws Exception {
		final Properties extraParams = new Properties();
		extraParams.setProperty(CAdESExtraParams.CONTENT_MIME_TYPE, MIMETYPE_JPEG);
		final byte[] signature = sign(this.jpegData, extraParams);
		final String mimeType = getMimeType(signature);

		Assertions.assertNull(mimeType, "No se debe agregar el mimetype a las firmas CAdES que no pidan incluirlo"); //$NON-NLS-1$
	}
	 ...
	 */
	@Test
	void cadesSinMimetype() throws Exception {
		final Properties extraParams = new Properties();
	 ...
	 */
	@Test
	void cadesSinMimeTypeIndicandolo() throws Exception {
		final Properties extraParams = new Properties();
		extraParams.setProperty(CAdESExtraParams.INCLUDE_MIMETYPE_ATTRIBUTE, Boolean.FALSE.toString());
		extraParams.setProperty(CAdESExtraParams.CONTENT_MIME_TYPE, MIMETYPE_JPEG);
		final byte[] signature = sign(this.jpegData, extraParams);
	...
	void cadesConMimeTypeIndicandolo() throws Exception {
		...
		Assertions.assertEquals(MIMETYPE_JPEG, mimeType, "No se ha agregado el MimeType indicado"); //$NON-NLS-1$
	}
