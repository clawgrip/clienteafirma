...
final class TestCosign {
	...
	@Test
	void pruebaCofirmarFirmaImplicitaSinIndicarDatos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(IMPLICIT_SHA1_SIGN_FILE)) {
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirmar_cades_T() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(IMPLICIT_SHA1_CADES_T_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}

		final byte[] originalData;
		try (InputStream is2 = getClass().getClassLoader().getResourceAsStream("FicheroOriginal_cades_t_csig.pdf")) { //$NON-NLS-1$
		...
		try {
			...
			Assertions.fail("La firma no puede ser cofirmada: " + e); //$NON-NLS-1$
			return;
		}
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirmar_firma_longeva_error() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(IMPLICIT_SHA1_CADES_A_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
		...
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(DATA_FILE_PDF)) {
			signer.cosign(
		...
		Assertions.fail("Deberia haber encontrado sellos y fallar"); //$NON-NLS-1$
	}

	/** Prueba de cofirma impl&iacute;cita de una firma impl&iacute;cita CAdES-LTA Level sin indicar los datos de firma.
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirmar_firma_longeva_ok() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(IMPLICIT_SHA1_CADES_A_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
		...
		try {
			...
			Assertions.fail("La firma se tuvo que haber cofirmado al incluir el parametro '" + CAdESExtraParams.ALLOW_SIGN_LTS_SIGNATURES + "': " + e); //$NON-NLS-1$ //$NON-NLS-2$
		}
		catch(final Exception e) {
	 ...
	 */
	@Test
	void pruebaCofirmarFirmaImplicitaIndicandoDatos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(IMPLICIT_SHA1_SIGN_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void pruebaCofirmaImplicitaDeFirmaExplicitaSinIndicarDatos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(EXPLICIT_SHA1_SIGN_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
	 ...
	 */
	@Test
	void pruebaCofirmaImplicitaDeFirmaExplicitaIndicandoDatos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(EXPLICIT_SHA1_SIGN_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirma_explicita_de_firma_explicita_indicando_hash() throws Exception {
		final byte[] sign;
		try (
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirma_explicita_de_firma_explicita_mismo_algoritmo_sin_datos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(EXPLICIT_SHA1_SIGN_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
	 ...
	 * @throws Exception Cuando se produce un error. */
	@Test
	void prueba_cofirma_explicita_de_firma_explicita_distinto_algoritmo_sin_datos() throws Exception {
		final byte[] sign;
		try (InputStream is = getClass().getClassLoader().getResourceAsStream(EXPLICIT_SHA1_SIGN_FILE)) {
			sign = AOUtil.getDataFromInputStream(is);
		}
		...
		Assertions.fail("La firma tuvo que haber lanzado una excepcion de tipo AOException"); //$NON-NLS-1$
	}
}
