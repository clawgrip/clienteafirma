...
final class TestEcdsa {
	...
	void signEcdsa() throws Exception {
		...
		try (InputStream is = TestEcdsa.class.getResourceAsStream(CRT)) {
			ks.load(is, PWD);
		}
		...
		final byte[] pdf;
		try (InputStream is = TestEcdsa.class.getResourceAsStream(PDF)) {
			pdf = AOUtil.getDataFromInputStream(is);
		}
