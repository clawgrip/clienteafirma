 ...
 * @version 0.3
 */
final class TestAOUtil {

	/** Prueba los m&eacute;todos de <code>Platform</code> */
	@SuppressWarnings("static-method")
	@Test
	void testPlatform() {
		final String libDir = Platform.getSystemLibDir();
		Assertions.assertNotNull(libDir);
	...
	@SuppressWarnings("static-method")
	@Test
	void testCreateUri() throws Exception {
		final URI uri = AOUtil.createURI("c:\\kaka\\naca\\/das des\\a.txt"); //$NON-NLS-1$
    ...
    void testGetCN() {
        ...
        for (final String[] principal : principals) {
            ...
            Assertions.assertEquals(AOUtil.getCN(principal[0]), principal[1], "El metodo getCN() no devuelve el resultado correcto para " + principal[0]); //$NON-NLS-1$
        }
    }
    ...
    @SuppressWarnings("static-method")
	@Test
    void testBase64ToProperties() throws IOException {
    	...
    	for (final String k : prop.keySet().toArray(new String[0])) {
    		final String property = prop.getProperty(k);
