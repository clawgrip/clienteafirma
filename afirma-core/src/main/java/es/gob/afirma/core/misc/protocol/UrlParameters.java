...
public abstract class UrlParameters {
	...
	void setCommonParameters(final Map<String, String> params) throws ParameterException {
		...
		// Comprobamos que se nos hayan indicado los datos o, en su defecto, el identificador de fichero
		// remoto a descargar y la ruta del servicio remoto para el fichero
		if (!params.containsKey(DATA_PARAM)) {
			...
			if (params.containsKey(FILE_ID_PARAM)) {
				...
				try {
					setRetrieveServletUrl(validateURL(params.get(RETRIEVE_SERVLET_PARAM)));
				}
			...
			if (dataPrm.startsWith("file:/")) { //$NON-NLS-1$
				throw new ParameterException("No se permite la lectura de ficheros locales: " + dataPrm); //$NON-NLS-1$
			}
			try {
				setData(
					DataDownloader.downloadData(
						dataPrm,
						// Boolean.parseBoolean() da false con null y en general con cualquier cosa que no sea la cadena "true"
						Boolean.parseBoolean(params.get(GZIPPED_DATA_PARAM))
					)
				);
			}
			catch (final Exception e) {
				throw new ParameterException("No se han podido obtener los datos", e); //$NON-NLS-1$
			}
		}
	}
	...
	protected static String getKeyStoreName(final Map<String, String> params) {
		...
		String ksValue = null;
		if (params.get(KEYSTORE_PARAM) != null) {
			try {
				ksValue = new String(Base64.getDecoder(). decode(params.get(KEYSTORE_PARAM)));
			}
			catch (final Exception e) {
	...
	private static String cleanupPath(final String path) {
		...
		try {
			return new File(cleanedpath).getCanonicalPath();
		}
		catch (final Exception e) {
