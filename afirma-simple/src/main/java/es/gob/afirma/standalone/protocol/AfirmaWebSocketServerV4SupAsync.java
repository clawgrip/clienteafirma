 ...
 */
public final class AfirmaWebSocketServerV4SupAsync extends AfirmaWebSocketServer {
	 ...
	 * las llamadas.
	 */
	public AfirmaWebSocketServerV4SupAsync(final int port, final String sessionId, final int protocol) {
	...
	public void onMessage(final WebSocket ws, final String message) {
		...
		if (message.startsWith(ECHO_REQUEST_PREFIX)) {
    		LOGGER.info("Se devuelve el ECHO de espera al websocket: " + ws.getRemoteSocketAddress().getAddress() //$NON-NLS-1$
    					+ " Que tiene el puerto: " + ws.getRemoteSocketAddress().getPort()); //$NON-NLS-1$
