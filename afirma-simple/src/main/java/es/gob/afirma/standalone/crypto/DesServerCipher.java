...
public class DesServerCipher implements ServerCipher{
	...
	public DesServerCipher(final byte[] key) throws JSONException, IOException {
		
		final String jsonString = new String(key, java.nio.charset.StandardCharsets.UTF_8);
		
		final JSONObject json = new JSONObject(jsonString);
		
		final byte[] jsonCipherKey = json.getString("key").getBytes(); //$NON-NLS-1$
		
		this.cipherKey = jsonCipherKey != null ? jsonCipherKey.clone() : null;
	}
	...
	@Override
	public String cipherData(final byte[] data) throws GeneralSecurityException {
		final Cipher desCipher;
		try {
			desCipher = Cipher.getInstance("DES/ECB/NoPadding"); //$NON-NLS-1$
		}
